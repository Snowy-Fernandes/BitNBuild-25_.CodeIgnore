...
# # Your Hugging Face model - with fallback approach
# MODEL_REPO = "ashal123/recipe-generator-ashal"
...
# SAMPLE_RECIPES = [
#     {
#         "id": 1, "title": "Spicy Thai Basil Chicken", "time": "20 min", "servings": "3", 
#         "calories": "385", "image": "🍗", "description": "Aromatic stir-fry with fresh basil",
#         "cuisine": "asian", "dietary": "regular", "difficulty": "medium"
#     },
#     {
#         "id": 2, "title": "Creamy Mushroom Risotto", "time": "35 min", "servings": "4", 
#         "calories": "420", "image": "🍄", "description": "Rich and comforting Italian classic",
#         "cuisine": "italian", "dietary": "vegetarian", "difficulty": "medium"
#     },
#     {
#         "id": 3, "title": "Vegan Buddha Bowl", "time": "25 min", "servings": "2", 
#         "calories": "320", "image": "🥗", "description": "Colorful plant-powered bowl",
#         "cuisine": "fusion", "dietary": "vegan", "difficulty": "easy"
#     },
#     {
#         "id": 4, "title": "Mexican Street Tacos", "time": "30 min", "servings": "4", 
#         "calories": "280", "image": "🌮", "description": "Authentic street-style tacos",
#         "cuisine": "mexican", "dietary": "regular", "difficulty": "easy"
#     },
#     {
#         "id": 5, "title": "Japanese Ramen Bowl", "time": "40 min", "servings": "2", 
#         "calories": "450", "image": "🍜", "description": "Hearty traditional ramen",
#         "cuisine": "asian", "dietary": "regular", "difficulty": "hard"
#     },
#     {
#         "id": 6, "title": "Mediterranean Quinoa Salad", "time": "15 min", "servings": "3", 
#         "calories": "290", "image": "🥙", "description": "Fresh and healthy salad",
#         "cuisine": "mediterranean", "dietary": "vegan", "difficulty": "easy"
#     }
# ]

# class RobustRecipeGenerator:
#     def __init__(self):
#         self.model = None
#         self.tokenizer = None
#         self.pipeline = None
...
#         models_to_try = [MODEL_REPO] + BACKUP_MODELS
...
#                 # Also load components separately
...
#             print("⚠️  No models could be loaded. Using template-based generation only.")
    
#     def generate_with_api(self, prompt, constraints):
#         """Try using Hugging Face Inference API as fallback"""
#         try:
#             API_URL = f"https://api-inference.huggingface.co/models/{MODEL_REPO}"
#             headers = {"Authorization": "Bearer YOUR_HF_TOKEN_HERE"}  # You'd need to add your token
...
#     def create_smart_prompt(self, user_prompt, constraints):
#         """Create an optimized prompt for recipe generation"""
#         dietary = constraints.get('dietary', 'regular')
#         cuisine = constraints.get('cuisine', 'any')
#         difficulty = constraints.get('difficulty', 'medium')
        
#         # Simple, clear prompt that works better with GPT models
#         prompt = f"""
#         Create a {dietary} {cuisine} recipe for: {user_prompt}
...
#     def parse_recipe_response(self, text, constraints):
#         """Parse model response into structured recipe format"""
#         try:
#             # Initialize with template
#             recipe = self.fallback_template(constraints, "Generated recipe")
            
#             # Try to extract title
#             title_match = re.search(r'Recipe Title:\s*(.+)', text, re.IGNORECASE)
#             if title_match:
...
#                 if ingredients:
#                     recipe['ingredients'] = ingredients[:8]  # Limit to 8 ingredients
...
#                 step_pattern = r'(\d+\.)\s*(.+?)(?=\n\d+\.|\n\n|$)'
#                 matches = re.findall(step_pattern, instructions_text, re.DOTALL)
#                 if matches:
#                     for num, instruction in matches:
#                         instructions.append(instruction.strip())
#                 else:
#                     # Fallback: split by lines
#                     lines = [line.strip() for line in instructions_text.split('\n') if line.strip()]
#                     instructions = [re.sub(r'^[\d\-\*\.\s]+', '', line).strip() for line in lines if line]
                
#                 if instructions:
#                     recipe['instructions'] = instructions[:6]  # Limit to 6 steps
            
#             recipe['model_generated'] = True
#             return recipe
            
#         except Exception as e:
#             print(f"Error parsing recipe: {e}")
#             return self.fallback_template(constraints, "Parsing failed")
    
#     def fallback_template(self, constraints, user_prompt):
...
#                 'bases': ['quinoa', 'brown rice', 'pasta', 'zucchini noodles', 'sweet potato'],
#                 'vegetables': ['spinach', 'kale', 'broccoli', 'bell peppers', 'mushrooms', 'carrots'],
#                 'styles': ['stir-fry', 'bowl', 'curry', 'salad', 'soup']
#             },
#             'vegetarian': {
#                 'proteins': ['paneer', 'eggs', 'cheese', 'yogurt', 'lentils', 'beans'],
#                 'bases': ['rice', 'pasta', 'bread', 'potatoes', 'quinoa', 'couscous'],
#                 'vegetables': ['tomatoes', 'spinach', 'mushrooms', 'zucchini', 'eggplant', 'onions'],
#                 'styles': ['bake', 'stir-fry', 'curry', 'casserole', 'salad']
#             },
#             'regular': {
#                 'proteins': ['chicken', 'beef', 'fish', 'shrimp', 'pork', 'tofu'],
#                 'bases': ['rice', 'pasta', 'potatoes', 'bread', 'quinoa', 'noodles'],
#                 'vegetables': ['carrots', 'broccoli', 'spinach', 'peppers', 'onions', 'garlic'],
#                 'styles': ['stir-fry', 'roast', 'grill', 'stew', 'bake']
#             }
#         }
        
#         template = templates.get(dietary, templates['regular'])
...
#         # Enhanced descriptions based on cuisine
#         cuisine_descriptions = {
#             'italian': f"Authentic {cuisine} style with fresh herbs",
#             'indian': f"Flavorful {cuisine} spices and aromatics",
#             'mexican': f"Vibrant {cuisine} flavors and colors",
#             'asian': f"Traditional {cuisine} techniques and balance",
#             'fusion': "Creative fusion of global flavors"
#         }
        
#         description = cuisine_descriptions.get(cuisine, "Delicious homemade recipe")
        
#         return {
#             'title': f"{cuisine.title()} {protein.title()} {style.title()}",
#             'description': f"{description}. {user_prompt}",
#             'ingredients': [
#                 f"250g {protein}, prepared",
#                 f"200g {base}",
#                 f"150g {vegetable}, chopped",
...
#             ],
#             'instructions': [
#                 f"Prepare all ingredients. Cook {base} according to package instructions.",
#                 f"Heat oil in a pan over medium heat. Add onions and garlic, cook until fragrant.",
#                 f"Add {protein} and cook until golden brown and cooked through.",
#                 f"Add {vegetable} and cook for 5-7 minutes until tender but crisp.",
#                 "Season with salt, pepper, and your choice of spices. Mix well.",
#                 f"Combine with cooked {base}. Garnish with fresh herbs and serve hot."
#             ],
#             'time': f"{random.randint(20, 45)} minutes",
#             'servings': random.randint(2, 6),
#             'difficulty': difficulty,
#             'calories': f"{random.randint(350, 550)}",
#             'tags': [dietary, cuisine, difficulty, style],
#             'external_links': {
#                 'zepto': f"https://www.zeptonow.com/search?q={protein}+{vegetable}+{base}",
#                 'blinkit': f"https://blinkit.com/search?q={protein}+{vegetable}+{base}"
#             },
#             'model_generated': False,
#             'template_used': True
#         }
    
#     def generate_recipe(self, user_prompt, constraints):
#         """Main recipe generation method with robust error handling"""
#         if not self.is_loaded:
#             print("🔄 Using high-quality template fallback")
#             return self.fallback_template(constraints, user_prompt)
        
#         try:
#             # Create optimized prompt
#             prompt = self.create_smart_prompt(user_prompt, constraints)
#             print(f"🔮 Generating with: {self.current_model}")
...
#                 recipe_text = generated_text.split(prompt)[-1].strip()
#             else:
#                 recipe_text = generated_text
            
#             print(f"📝 Generated {len(recipe_text)} characters")
            
#             # Parse the result
#             recipe = self.parse_recipe_response(recipe_text, constraints)
#             recipe['image'] = random.choice(['🍕', '🍝', '🌮', '🍛', '🥗', '🍜', '🍗', '🥘'])
#             recipe['generation_model'] = self.current_model
...
#             # Try API fallback
#             api_result = self.generate_with_api(prompt, constraints)
#             if api_result:
#                 return self.parse_recipe_response(api_result, constraints)
#             else:
#                 return self.fallback_template(constraints, user_prompt)
...
# recipe_generator = RobustRecipeGenerator()

# @app.route('/api/health', methods=['GET'])
# def health_check():
#     return jsonify({
#         'status': 'healthy',
#         'model_loaded': recipe_generator.is_loaded,
#         'current_model': recipe_generator.current_model or 'template',
#         'message': 'API is running with fallback support'
#     })
...
#         'model_status': recipe_generator.current_model or 'template'
#     })
...
#         recipe = recipe_generator.generate_recipe(prompt, constraints)
#         recipe['id'] = int(datetime.now().timestamp())
        
#         # Get suggested recipes
#         suggested = [r for r in SAMPLE_RECIPES if r['id'] != recipe.get('id')]
#         suggested_recipes = random.sample(suggested, min(3, len(suggested)))
        
#         return jsonify({
#             'success': True,
#             'recipe': recipe,
#             'suggested_recipes': suggested_recipes,
#             'generation_method': recipe.get('generation_model', 'template')
#         })
...
#             'recipe': recipe_generator.fallback_template({}, 'Error recovery')
#         })

# @app.route('/api/test-generation', methods=['POST'])
# def test_generation():
#     """Test endpoint for recipe generation"""
#     test_prompt = "vegan pizza with mushrooms and olives"
#     test_constraints = {'dietary': 'vegan', 'cuisine': 'italian'}
    
#     recipe = recipe_generator.generate_recipe(test_prompt, test_constraints)
    
#     return jsonify({
#         'test_prompt': test_prompt,
#         'result': recipe,
#         'model_used': recipe.get('generation_model', 'template')
#     })

# if __name__ == '__main__':
#     print("🚀 Starting Robust Recipe Generator API...")
#     print("💡 Features: Model fallbacks + High-quality templates")
...
def home():
    return jsonify({
        ...
        "version": "1.0.0",
        "endpoints": {
            "/api/options": "GET - Get available options",
            "/api/generate-recipe": "POST - Generate recipes",
...
if __name__ == '__main__':
    print("🍳 Starting AI Recipe Generator API...")
    print("📚 Available endpoints:")
    print("   GET  /api/options - Get available options")
    print("   POST /api/generate-recipe - Generate 5 recipe variations")
    print("   POST /api/generate-meal-plan - Generate meal plan")
    print("   GET  /api/health - Health check")
    print("\n🎯 Features:")
    print("   - Recipe 1: Uses your specified ingredients")
    print("   - Recipe 2-5: Increasingly creative variations")
    print("   - Different ingredient combinations in each recipe")
    print("   - Progressive complexity and creativity")
